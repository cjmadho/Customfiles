...
#          1.4 03/01/2018 (fixed backuop script issue where removal of old files wasn't working)
# =============================================================================
...
tail -n 1 "/oxa/oxa-tools5/scripts/backup_configuration_mysql.sh" |wc -c|xargs -I {} truncate "/oxa/oxa-tools5/scripts/backup_configuration_mysql.sh" -s -{}
echo "AZURE_STORAGEACCOUNT_CONNECTIONSTRING=\"DefaultEndpointsProtocol=https;AccountName=risedulrnbackups;AccountKey=Ucy7jysAPEj2vWU72YtPAWM67UnMvlVjzUmdETurGcMF+N7Lr38PB0LCYgeC9WvRm5qz+ptmXEXvzwXBdfN7vQ==;EndpointSuffix=core.windows.net\"" >> /oxa/oxa-tools5/scripts/backup_configuration_mysql.sh
...
tail -n 1 "/oxa/oxa-tools5/scripts/backup_configuration_mongo.sh" |wc -c|xargs -I {} truncate "/oxa/oxa-tools5/scripts/backup_configuration_mongo.sh" -s -{}
echo "AZURE_STORAGEACCOUNT_CONNECTIONSTRING=\"DefaultEndpointsProtocol=https;AccountName=risedulrnbackups;AccountKey=Ucy7jysAPEj2vWU72YtPAWM67UnMvlVjzUmdETurGcMF+N7Lr38PB0LCYgeC9WvRm5qz+ptmXEXvzwXBdfN7vQ==;EndpointSuffix=core.windows.net\"" >> /oxa/oxa-tools5/scripts/backup_configuration_mongo.sh
...
sed -i '137s/.*/    CONTAINER_PREF=`echo "${HOSTNAME,,}" | rev | cut -c3- | rev`/' /oxa/oxa-tools5/scripts/db_backup.sh
sed -i '138s/.*/    CONTAINER_NAME="${DATABASE_TYPE}-backup"/' /oxa/oxa-tools5/scripts/db_backup.sh
sed -i '259s/.*/        sc=$(az storage container show --connection-string "${AZURE_STORAGE_CONNECTIONSTRING}" --name "${CONTAINER_PREF}-${CONTAINER_NAME}" -o json)/' /oxa/oxa-tools5/scripts/db_backup.sh
sed -i '269s/.*/            response=$(az storage container create --connection-string "${AZURE_STORAGE_CONNECTIONSTRING}" --name "${CONTAINER_PREF}-${CONTAINER_NAME}" -o json)/' /oxa/oxa-tools5/scripts/db_backup.sh
sed -i '299s/.*/        result=$(az storage blob upload --connection-string "${AZURE_STORAGE_CONNECTIONSTRING}" --file "${COMPRESSED_FILE}" --container-name "${CONTAINER_PREF}-${CONTAINER_NAME}" --name "${COMPRESSED_FILE}" -o json)/' /oxa/oxa-tools5/scripts/db_backup.sh
sed -i '337s/.*/    CONTAINER_NAME=${CONTAINER_PREF}-$CONTAINER_NAME}/' /oxa/oxa-tools5/scripts/db_backup.sh
}
